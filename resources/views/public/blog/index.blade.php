                            ...
                            <a href="{{ $post['url'] }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5B1121] focus-visible:ring-offset-4 focus-visible:ring-offset-[#FAF7F2]">
                                ...
                                @else
                                    <div class="aspect-16/10 bg-[#211B19] flex items-center justify-center px-8 text-center">
                                        <span class="font-serif text-2xl font-bold text-[#C5A880] break-words">{{ $post['title'] }}</span>
                                    </div>
                                @endif
                            </a>
                            ...
                            <div class="flex flex-1 flex-col justify-between p-6 sm:p-7">
                                <div class="space-y-3">
                                    ...
                                    <h2 class="font-serif text-2xl font-bold leading-snug tracking-normal text-[#5B1121] break-words">
                                        <a href="{{ $post['url'] }}" class="hover:text-[#B3956B] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5B1121] focus-visible:ring-offset-4">
                                            {{ $post['title'] }}
                                        </a>
                                    </h2>
                                ...
                                <div class="mt-6 flex items-center justify-between gap-4 border-t border-brand-border pt-5">
                                    @if(!empty($post['published_at_display']))
                                        <time datetime="{{ $post['published_at_iso'] }}" class="text-sm text-brand-text-muted">
                                            {{ $post['published_at_display'] }}
                                        </time>
                                    @endif

                                    <a href="{{ $post['url'] }}" class="ml-auto inline-flex items-center text-xs font-semibold uppercase tracking-widest text-[#5B1121] hover:text-[#B3956B] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5B1121] focus-visible:ring-offset-4">
                                        {{ __('blog.index.details') }}
                                    </a>
                                </div>
                            </div>
                        </article>
