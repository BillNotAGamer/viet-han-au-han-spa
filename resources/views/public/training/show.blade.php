        ...
        <section class="relative bg-[#181312] pt-28 sm:pt-32 lg:pt-36 text-white overflow-hidden">
            @if(!empty($course['hero_media']['url']))
                <div class="absolute inset-0" data-reveal-image>
                    <img src="{{ $course['hero_media']['url'] }}" alt="{{ $course['hero_media']['alt'] }}" class="h-full w-full object-cover" loading="eager" fetchpriority="high">
                                    ...
                                    @foreach($course['curriculum_modules'] as $module)
                                        <div class="grid grid-cols-[48px_minmax(0,1fr)] gap-5 rounded-2xl border border-[#E8DFC8] bg-white p-6 shadow-xs transition duration-200 hover:border-[#C5A880]" data-reveal>
                                            ...
                                            <div class="min-w-0 pt-0.5">
                                                @if($module['title'] !== '')
                                                    <h3 class="text-xl sm:text-[22px] font-semibold text-[#5B1121] break-words">{{ $module['title'] }}</h3>
                                                @endif
                                                @if($module['description'] !== '')
                                                    <p class="mt-2 text-[17px] leading-[1.65] text-[#554D4A] break-words">{{ $module['description'] }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            <dl class="mt-5 divide-y divide-[#E8DFC8]">
                                @if(!empty($course['tuition']))
                                    <div class="py-3.5 first:pt-0 last:pb-0">
                                        <dt class="text-xs font-semibold uppercase tracking-widest text-[#9B7B4F]">{{ __('training.detail.tuition') }}</dt>
                                        <dd class="mt-1 font-semibold text-[#5B1121] text-xl break-words">{{ $course['tuition']['display'] }}</dd>
                                    </div>
                                @endif

                                @if(!empty($course['duration_display']))
                                    <div class="py-3.5 first:pt-0 last:pb-0">
                                        <dt class="text-xs font-semibold uppercase tracking-widest text-[#9B7B4F]">{{ __('training.detail.duration') }}</dt>
                                        <dd class="mt-1 text-[15px] sm:text-[16px] text-[#554D4A] break-words">{{ $course['duration_display'] }}</dd>
                                    </div>
                                @endif

                                @if(!empty($course['schedule_display']))
                                    <div class="py-3.5 first:pt-0 last:pb-0">
                                        <dt class="text-xs font-semibold uppercase tracking-widest text-[#9B7B4F]">{{ __('training.detail.schedule') }}</dt>
                                        <dd class="mt-1 text-[15px] sm:text-[16px] text-[#554D4A] break-words">{{ $course['schedule_display'] }}</dd>
                                    </div>
                                @endif

                                @if(!empty($course['target_audience']))
                                    <div class="py-3.5 first:pt-0 last:pb-0">
                                        <dt class="text-xs font-semibold uppercase tracking-widest text-[#9B7B4F]">{{ __('training.detail.target_audience') }}</dt>
                                        <dd class="mt-1 text-[15px] sm:text-[16px] text-[#554D4A] break-words">{{ $course['target_audience'] }}</dd>
                                    </div>
                                @endif
                            </dl>
                        </section>
